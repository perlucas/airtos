...
# Possible Actions the agent has to choose from
ACTION_NOOP = 0
ACTION_WEAK_BUY = 1
...
# Percentages of either budget or shares affected by action
ACTION_PERCENTAGES = [
    0,      # noop
...
# Representative colors for each action, mapped by index. Used for rendering actions
COLOR_CODES = [
    None,       # noop
    '#6AF26F',  # weak buy
    '#59C05C',  # regular buy
...
class TradingEnv(py_environment.PyEnvironment):
    ...
    Former class has been adapted to fit in the required methods for TensorFlow environments
    ...
    def _step(self, action):
        ...
        if self._current_tick == self._end_tick or not self._can_still_operate():
            # End episode if reached last possible tick or can't operate
    ...
    def _compute_new_shares(self, action, price):
        ...
        if is_sell:
            # Compute operation shares over current shares
        ...
        operation_shares = np.floor(
            (budget * ACTION_PERCENTAGES[action]) / price)  # Number of shares to buy
        return (self._shares + operation_shares, operation_shares)
    ...
    def _can_still_operate(self):
        ...
        min_shares_to_buy = np.floor(
            budget * ACTION_PERCENTAGES[ACTION_WEAK_BUY] / min_price)

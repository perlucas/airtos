...
class TradingEnv(py_environment.PyEnvironment):
    ...
    def __init__(self, df, window_size, frame_bound):
        ...
        num_inputs = window_size * self.signal_features.shape[1]
        self._observation_spec = array_spec.BoundedArraySpec(
            shape=(num_inputs,),
            dtype=np.float32,
            minimum=[-1000] * num_inputs,
    ...
    def _get_observation(self):
        ...
        return np.reshape(self.signal_features[(self._current_tick-self.window_size+1):self._current_tick+1], (num_inputs, ))
    ...
    def _compute_new_shares(self, action, price):
        ...
        if is_sell:
            ...
            operation_shares = np.floor(self._shares * ACTION_PERCENTAGES[action])
        ...
        operation_shares = np.floor(
            (budget * ACTION_PERCENTAGES[action]) / price)  # number of shares to operate with
        return (self._shares + operation_shares, operation_shares)

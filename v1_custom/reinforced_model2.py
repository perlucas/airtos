...
class StockPriceAccumulator:
    ...
    def get_reward(self, price_variation):
        return price_variation * (self.buys - self.sells) * 10
...
for episode in range(num_episodes):
    episode_index = num_episodes % len(input_data)
    total_reward=0
    acc = StockPriceAccumulator()

    for step in range(99):
        # state=input_data[episode][step]
        state=input_data[episode_index][step]
        ...
        next_state=input_data[episode_index][step + 1]
